...
const glassBg =
  "linear-gradient(0deg, rgba(75, 83, 32, 0.15), rgba(75, 83, 32, 0.15)), linear-gradient(0deg, rgba(0, 0, 0, 0.25), rgba(0, 0, 0, 0.25)), linear-gradient(0deg, rgba(255, 255, 255, 0.1), rgba(255, 255, 255, 0.1))";
...
export default function StoryPopup({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      onClick={handleClose}
      className={`fixed inset-0 z-50 flex items-center justify-center bg-black/5 p-0 backdrop-blur-xs transition-opacity duration-300 md:p-6 ${
        show ? "opacity-100" : "opacity-0"
      }`}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className={`@container relative flex h-svh w-full flex-col overflow-hidden border-white/40 shadow-[0_8px_60px_rgba(0,0,0,0.35)] backdrop-blur-xs transition-all duration-500 ease-out md:h-[90vh] md:max-h-155 md:max-w-290 md:rounded-[28px] md:border ${
      ...
      >
        ...
        <div className="flex flex-col  items-center px-6 pt-2  md:pt-6">
          ...
          <h2
            className="mt-2 text-center text-xl md:text-[26px] font-semibold leading-8.5 tracking-normal text-white"
            style={{ fontFamily: "var(--font-inter)", textShadow: "0px 4px 4px #000000" }}
          >
        ...
        <div className="flex flex-1 flex-col  items-center justify-evenly gap-5 overflow-y-auto px-6 pb-5 pt-4 @2xl:flex-row @2xl:items-center @2xl:gap-8 @2xl:overflow-hidden @2xl:px-10">
          ...
          <div className="flex w-full min-h-0 flex-col @2xl:w-[58%]">
            ...
            />
            <p
              className="mt-4 text-center text-[16px] md:text-[20px] font-semibold leading-6 tracking-normal text-white"
              style={{ fontFamily: "var(--font-inter)", textShadow: "0px 3px 6px rgba(0,0,0,0.4)" }}
            >
          ...
          <div className="relative flex  w-full md:-mt-32 min-h-0 justify-center @2xl:w-[42%]">
            <div
              className="relative z-10 rounded-3xl  border border-white/40 p-5 "
              style={{ background: glassBg }}
            >
              <Image src="/quate.png" alt="" width={19} height={19} className="absolute -top-3 left-4 w-4 md:w-6 object-contain" />
              <h3
                className="text-xl md:text-[23.93px] font-semibold leading-none tracking-normal text-white"
